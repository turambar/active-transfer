...
Xt = X10v19(split10v19~=3,:);
yt = y10v19(split10v19~=3);

Src{1}.X = X10v19(split10v19==3,:);
Src{1}.y = y10v19(split10v19==3);
Src{2}.X = X11v20(split11v20==3,:);
Src{2}.y = y11v20(split11v20==3);
Src{3}.X = X08v16(split08v16==3,:);
Src{3}.y = y08v16(split08v16==3);
alphas = [ 0.4 0.6 0.8 ]
...
save('icdm-20newsgroup.mat', 'alphas', 'kfolds', 'reps', 'parts', ...
...
plot(0:step:budget, iwal_queries_mean(1:step:end), 'k-', 'LineWidth', 2)

f2 = figure;
hold on;
plot([ 0 budget ], [ acc_mean_sl acc_mean_sl ], 'k:', 'LineWidth', 2)
plot(iwal_queries_mean(1:step:end), iwal_acc_mean(1:step:end), 'k-', 'LineWidth', 2)
...
for src=1:numel(Src)
    ...
    figure(f1)
    hold on
    plot(0:step:budget, tiwal_queries_mean(1:step:end), sprintf('%s-', cs(src)), 'LineWidth', 2)
    ...
    plot(tiwal_queries_mean(1:step:end), tiwal_acc_mean(1:step:end), sprintf('%s-', cs(src)), 'LineWidth', 2)
end
